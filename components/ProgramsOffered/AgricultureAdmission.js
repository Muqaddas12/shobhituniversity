...
const AgricultureAdmission = () => {
  // State to manage the expanded/collapsed state of the B.Sc. (Hons.) program section
  const [bscHonsExpanded, setBscHonsExpanded] = useState(false);

  // Function to toggle the expansion state of the B.Sc. (Hons.) program
  const toggleProgram = () => {
    setBscHonsExpanded(!bscHonsExpanded);
  };

  const applyLink = 'https://sug.ac.in/school-of-agriculture/admission.php'; // Placeholder link, adjust as needed

  return (
    <ScrollView style={styles.container}>
      <Text style={styles.mainTitle}>School of Agriculture and Environmental Sciences</Text>

      {/* B.Sc. (Hons.) Program Section */}
      <View style={styles.programCard}>
        <TouchableOpacity onPress={toggleProgram} style={styles.programTitleContainer}>
          <Text style={styles.programTitle}>B.Sc. (Hons.)</Text>
          <Text style={styles.toggleIcon}>{bscHonsExpanded ? '▲' : '▼'}</Text>
        </TouchableOpacity>

        {bscHonsExpanded && ( // Conditionally render content if expanded
          <View style={styles.programContent}>
            <Text style={styles.detailText}>Agriculture</Text>
            ...
            <Text style={styles.detailText}>
              Admission to B.Sc. Programs is open to candidates who have Passed (10+2) / Intermediate / Higher Secondary from any approved Board with PCM/PCB/Agriculture.
            </Text>
...
const styles = StyleSheet.create({
  ...
  websiteButton: { // Included for consistency, though not explicitly requested for Agriculture
